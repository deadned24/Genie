...
search for __genie_invalid_effects map. 
...
foreach eff in __genie_invalid_effects_list{
	__genie_invalid_effects[eff]=true;		
}
...
monster [int] genieGenerateValidMonsterList()
{
	...
	{
		boolean [monster] invalid_monsters_2;
		foreach s in __genie_invalid_monster_strings
		{
			monster m = s.to_monster();
			if (m == $monster[none]) continue;
			invalid_monsters_2[m] = true;
		}
		foreach m in $monsters[]
		{
			...
			if (invalid_monsters_2[m]) continue;
...
{
	return genericiseImageString((e.image.contains_text("s3.amazon") ? "" : "itemimages/") + e.image);
}
...
buffer genieGenerateNextEffectWishes()
{
	...
	if (true)
	{
		boolean [string] prismatic_res_modifiers;
		foreach e in $elements[hot,stench,spooky,cold,sleaze]
		{
			prismatic_res_modifiers[e + " Resistance"] = true;
		}
		modifier_buttons.listAppend(ModifierButtonEntryMake("Prismatic Res", prismatic_res_modifiers, 0, false, "itemimages/rrainbow.gif"));
		modifier_buttons[modifier_buttons.count() - 1].maximum_minimum = true;
	}
